...
//         <Button type="primary" htmlType="submit" block onClick={showModal}>
//           Get Started
//         </Button>
//       </div>
//       <Modal
...
const App = () => {
  ...
  const handleNext = () => {
    if (step === 1 && !accountName) {
      message.warning("Please enter the account name.");
      return;
    }
    setStep(2);
  };
  ...
  const handleAccountSubmit = () => {
    // if (!accountName) {
    //   message.warning("Account name cannot be empty!");
    //   return;
    // }
    message.success(`Account details added successfully!`);
    setStep(2); // Move to next step after submitting account details
  };
  ...
  const handleDepartmentSubmit = () => {
    ...
    setIsModalVisible(false); // Close modal after submitting department details
